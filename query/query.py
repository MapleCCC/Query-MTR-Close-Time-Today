...
class TrainServiceInfo:
    ...
    def IsReleasedToday(self) -> bool:
        # TODO: handle comparison beween aware date and naive date.
        # TODO: Replace locale-related today() with current time in Hong Kong area.
...
def brute_force_query(verbose: bool=False) -> str:
    ...
    if match:
        ...
        return("No train service information related to early close "
               "of train service is posted today.")
...
def normal_mode_query(verbose: bool=False) -> str:
    ...
    if verbose:
        print(f"Found {flip_area.number_of_train_service_info} train service info in flip area")

    for tsi in flip_area.sorted_tsi():
        if not tsi.IsReleasedToday():
            print("No train service information related to early close "
                  "of train service is posted today.")
            return

        text = tsi.get_sliding_text()
        matches = re.findall(remove_capture_group_from_pattern(CLOSE_TIME_ANNOUNCEMENT_PATTERN), text)
